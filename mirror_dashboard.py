...
API_KEY = "25e8a482b383f7c1611f9f6706ac1c57"  # your real OpenWeatherMap API key
...
def get_weather():
    ...
    try:
        ...
        for item in res["list"]:
            ...
            day = datetime.datetime.strptime(date_txt, "%Y-%m-%d").strftime("%a")
            if day not in seen:
                seen.add(day)
                days.append({
                    "day": day,
        ...
        weather_data = {"now": now_weather, "forecast": days[1:3]}
...
def draw_display():
    ...
    draw.text((300, 60), time_str, font=font_large, fill="gray")
    draw.text((300, 120), date_str, font=font_med, fill="gray")
    ...
    if weather_data is None:
        draw.text((x_offset, y_offset), "Loading weather...", font=font_med, fill="gray")
    elif "error" in weather_data:
        draw.text((x_offset, y_offset), "No Internet – Retrying...", font=font_med, fill="gray")
    else:
        w = weather_data["now"]
        draw.text((x_offset, y_offset), f"{round(w['temp'])}°F — {w['desc']}", font=font_med, fill="gray")
        draw.text((x_offset, y_offset + 35),
